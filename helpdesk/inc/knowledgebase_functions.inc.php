...
function hesk_kbTopArticles($how_many, $index = 1)
{
	...
	$sql = "SELECT `t1`.* FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` AS `t1`
			LEFT JOIN `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_categories` AS `t2` ON `t1`.`catid` = `t2`.`id`
			WHERE `t1`.`type`='0' AND `t2`.`type`='0'
			ORDER BY `t1`.`sticky` DESC, `t1`.`views` DESC, `t1`.`art_order` ASC LIMIT ".hesk_dbEscape($how_many);
...
function hesk_kbLatestArticles($how_many, $index = 1)
{
	...
	$sql = "SELECT `t1`.* FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` AS `t1`
			LEFT JOIN `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_categories` AS `t2` ON `t1`.`catid` = `t2`.`id`
			WHERE `t1`.`type`='0' AND `t2`.`type`='0'
			ORDER BY `t1`.`dt` DESC LIMIT ".hesk_dbEscape($how_many);
    $res = hesk_dbQuery($sql);
...
function hesk_kbSearchLarge($admin = '')
{
	...
	<div style="text-align:center">
		...
		<span class="largebold"><?php echo $hesklang['ask']; ?></span> <input type="text" name="search" size="50" class="large" />
...
function hesk_kbSearchSmall()
{
	...
	<td style="text-align:right" valign="top" width="300">
		<div style="display:inline;">
			...
			<input type="submit" value="<?php echo $hesklang['search']; ?>" class="greenbutton" onmouseover="hesk_btn(this,'greenbuttonover');" onmouseout="hesk_btn(this,'greenbutton');" />
			</form>
		</div>
	</td>
