...
function hesk_testMySQL()
{
	...
	// Confirm that all required database tables exist
	$tables = array(
...
function hesk_testPOP3()
{
	...
	$set['pop3_host_name']	= isset($_POST['s_pop3_host_name']) ? hesk_input($_POST['s_pop3_host_name']) : 'mail.domain.com';
	$set['pop3_host_port']	= isset($_POST['s_pop3_host_port']) ? intval($_POST['s_pop3_host_port']) : 110;
	$set['pop3_tls']		= empty($_POST['s_pop3_tls']) ? 0 : 1;
	$set['pop3_user']		= isset($_POST['s_pop3_user']) ? hesk_input($_POST['s_pop3_user']) : '';
	$set['pop3_password']	= isset($_POST['s_pop3_password']) ? hesk_input($_POST['s_pop3_password']) : '';
	...
	{
		// Authenticate
		if(($error=$pop3->Login($set['pop3_user'], $set['pop3_password']))=="")
		{
			if(($error=$pop3->Close()) == "")
			{
				// Connection OK
...
function hesk_testSMTP()
{
	...
	$set['smtp_host_port']	= isset($_POST['s_smtp_host_port']) ? intval($_POST['s_smtp_host_port']) : 25;
	$set['smtp_timeout']	= isset($_POST['s_smtp_timeout']) ? intval($_POST['s_smtp_timeout']) : 10;
	$set['smtp_ssl']		= empty($_POST['s_smtp_ssl']) ? 0 : 1;
	$set['smtp_tls']		= empty($_POST['s_smtp_tls']) ? 0 : 1;
	$set['smtp_user']		= isset($_POST['s_smtp_user']) ? hesk_input($_POST['s_smtp_user']) : '';
	$set['smtp_password']	= isset($_POST['s_smtp_password']) ? hesk_input($_POST['s_smtp_password']) : '';
	...
	$smtp->password		= $set['smtp_password'];
	$smtp->debug		= 1;
