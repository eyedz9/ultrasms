...
function edit_user()
{
    ...
    if ( ! isset($_SESSION['save_userdata']))
    {
		$sql = 'SELECT `user`,`pass`,`isadmin`,`name`,`email`,`signature`,`categories`,`autoassign`,`heskprivileges` AS `features` FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'users` WHERE `id`='.hesk_dbEscape($id).' LIMIT 1';
		$res = hesk_dbQuery($sql);
...
function new_user() {
	...
	$sql = "INSERT INTO `".hesk_dbEscape($hesk_settings['db_pfix'])."users` (`user`,`pass`,`isadmin`,`name`,`email`,`signature`,`categories`,`autoassign`,`heskprivileges` $sql_where) VALUES (
	...
	'".hesk_dbEscape($myuser['autoassign'])."',
...
function update_user() {
    ...
    /* Check for duplicate usernames */
	$sql = 'SELECT `id`,`isadmin`,`categories`,`heskprivileges` FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'users` WHERE `user` = \''.hesk_dbEscape($myuser['user']).'\' LIMIT 1';
	$res = hesk_dbQuery($sql);
    ...
    `autoassign`='".hesk_dbEscape($myuser['autoassign'])."',
...
function hesk_validateUserInfo($pass_required = 1, $redirect_to = './manage_users.php')
{
    ...
    $myuser['autoassign'] = isset($_POST['autoassign']) && $_POST['autoassign'] == 'Y' ? 1 : 0;
