...
function mail_delete()
{
	...
	if ($ids)
	{
		foreach ($ids as $id)
        {
	        ...
	        $sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'mail` SET `deletedby`='.hesk_dbEscape($_SESSION['id']).' WHERE `id`='.intval($id).' AND (`to`='.intval($_SESSION['id']).' OR `from`='.intval($_SESSION['id']).') AND `deletedby`=0 LIMIT 1';
	        $res = hesk_dbQuery($sql);

            if (hesk_dbAffectedRows() != 1)
            {
		        $sql = 'DELETE FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'mail` WHERE `id`='.intval($id).' AND (`to`='.intval($_SESSION['id']).' OR `from`='.intval($_SESSION['id']).') AND `deletedby`!=0 LIMIT 1';
		        $res = hesk_dbQuery($sql);
            }
        }
...
function mail_mark_unread()
{
	...
	if ($ids)
	{
		foreach ($ids as $id)
        {
	        $sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'mail` SET `read`=\'0\' WHERE `id`='.intval($id).' AND `to`='.intval($_SESSION['id']).' LIMIT 1';
	        hesk_dbQuery($sql);
        }
...
function mail_mark_read()
{
	...
	if ($ids)
	{
		foreach ($ids as $id)
        {
	        $sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'mail` SET `read`=\'1\' WHERE `id`='.intval($id).' AND `to`='.intval($_SESSION['id']).' LIMIT 1';
	        hesk_dbQuery($sql);
        }
...
function mail_send()
{
	...
	{
		$sql = "SELECT `name`,`email`,`notify_pm` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."users` WHERE `id`=".intval($_SESSION['mail']['to'])." LIMIT 1";
		$res = hesk_dbQuery($sql);
...
function show_message()
{
	global $hesk_settings, $hesklang, $admins;
		...
		$sql = "SELECT * FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."mail` WHERE `id`=".intval($id)." AND `deletedby`!=".intval($_SESSION['id'])." LIMIT 1";
		$res = hesk_dbQuery($sql);
		$num = hesk_dbNumRows($res);
	    ...
	    if ($num)
	    {
	        ...
	        {
				$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."mail` SET `read`='1' WHERE `id`=".intval($id)." LIMIT 1";
				$res = hesk_dbQuery($sql);
	        }
...
function mail_list_messages()
{
	...
	$sql = "SELECT COUNT(*) FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."mail` WHERE `".hesk_dbEscape($hesk_settings['mailtmp']['this'])."`=".intval($_SESSION['id'])." AND `deletedby`!=".intval($_SESSION['id']);
	$res = hesk_dbQuery($sql);
