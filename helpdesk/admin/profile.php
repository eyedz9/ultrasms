...
?>
	...
	if ($hesk_settings['can_sel_lang'])
	{
		...
		{
			$newlang = hesk_input($_GET['language']);
			...
			if ( isset($hesk_settings['languages'][$newlang]) )
			{
				...
				$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."users` SET `language`=$newlang WHERE `id`='".hesk_dbEscape($_SESSION['id'])."' LIMIT 1";
				hesk_dbQuery($sql);
			}
		}
...
function update_profile() {
    ...
    if ($_SESSION['isadmin'])
    {
		$_SESSION['new']['user']  = hesk_input($_POST['user']) or $hesk_error_buffer .= '<li>' . $hesklang['enter_username'] . '</li>';

	    /* Check for duplicate usernames */
		$sql = "SELECT `id` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."users` WHERE `user`='".hesk_dbEscape($_SESSION['new']['user'])."' AND `id`!= ".hesk_dbEscape($_SESSION['id'])." LIMIT 1";
    ...
    /* After reply */
    $_SESSION['new']['afterreply'] = isset($_POST['afterreply']) ? intval($_POST['afterreply']) : 0;
    ...
    else
    {
        ...
        `autostart`='".hesk_dbEscape($_SESSION['new']['autostart'])."' ,
        ...
        `notify_note`='".hesk_dbEscape($_SESSION['new']['notify_note'])."'
