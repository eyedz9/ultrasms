...
if (hesk_checkPermission('can_view_tickets',0))
{
	...
	if ( isset($_GET['reset']) && hesk_token_check($_GET['token']) )
	{
		$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."users` SET `default_list`='' WHERE `id` = '".hesk_dbEscape($_SESSION['id'])."' LIMIT 1";
