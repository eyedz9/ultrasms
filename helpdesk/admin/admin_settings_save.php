...
$set['noreply_name']	= hesk_input($_POST['s_noreply_name']);
...
$set['max_open']		= ! isset($_POST['s_max_open']) ? 0 : intval($_POST['s_max_open']);
$set['max_open']		= hesk_checkMinMax($set['max_open'],0,999,0);
...
if ($set['attachments']['use'])
{
    ...
    $size = floatval($_POST['s_max_size']) or $size = 1.0;
    $unit = hesk_htmlspecialchars($_POST['s_max_unit']) or $unit = 'MB';
...
else
{
	...
	$set['smtp_host_port']	= isset($_POST['tmp_smtp_host_port']) ? intval($_POST['tmp_smtp_host_port']) : 25;
	$set['smtp_timeout']	= isset($_POST['tmp_smtp_timeout']) ? intval($_POST['tmp_smtp_timeout']) : 10;
	$set['smtp_ssl']		= empty($_POST['tmp_smtp_ssl']) ? 0 : 1;
	$set['smtp_tls']		= empty($_POST['tmp_smtp_tls']) ? 0 : 1;
	$set['smtp_user']		= isset($_POST['tmp_smtp_user']) ? hesk_input($_POST['tmp_smtp_user']) : '';
	$set['smtp_password']	= isset($_POST['tmp_smtp_password']) ? hesk_input($_POST['tmp_smtp_password']) : '';
}
...
{
	$set['pop3_host_name']	= isset($_POST['tmp_pop3_host_name']) ? hesk_input($_POST['tmp_pop3_host_name']) : 'mail.domain.com';
	$set['pop3_host_port']	= isset($_POST['tmp_pop3_host_port']) ? intval($_POST['tmp_pop3_host_port']) : 110;
	$set['pop3_tls']		= empty($_POST['tmp_pop3_tls']) ? 0 : 1;
	$set['pop3_user']		= isset($_POST['tmp_pop3_user']) ? hesk_input($_POST['tmp_pop3_user']) : '';
	$set['pop3_password']	= isset($_POST['tmp_pop3_password']) ? hesk_input($_POST['tmp_pop3_password']) : '';
}

/* --> Email loops */
$set['loop_hits']		= empty($_POST['s_loop_hits']) ? 5 : intval($_POST['s_loop_hits']);
$set['loop_hits']		= hesk_checkMinMax($set['loop_hits'],0,999,5);
$set['loop_time']		= empty($_POST['s_loop_time']) ? 300 : intval($_POST['s_loop_time']);
$set['loop_time']		= hesk_checkMinMax($set['loop_time'],1,86400,300);

/* --> Detect email typos */
$set['detect_typos']	= empty($_POST['s_detect_typos']) ? 0 : 1;
$set['email_providers'] = array();

if ( ! empty($_POST['s_email_providers']) )
{
...
for ($i=1;$i<=20;$i++)
{
	...
	if ($set['custom_fields'][$this_field]['use'])
	{
		...
		$set['custom_fields'][$this_field]['type']		= isset($_POST['s_custom'.$i.'_type']) ? hesk_htmlspecialchars($_POST['s_custom'.$i.'_type']) : 'text';
	...
	while (false !== ($subdir = readdir($path)))
	{
		...
		if (filetype($dir . $subdir) == 'dir')
		{
	        ...
	        if (file_exists($langu))
	        {
                ...
                elseif ( ! preg_match('/\$hesklang\[\'updates2\'\]/', $tmp) )
                {
                	$add = 0;
                }
	        }
