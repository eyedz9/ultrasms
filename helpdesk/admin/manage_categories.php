...
function new_cat() {
    ...
    /* Options */
    $_SESSION['cat_autoassign'] = isset($_POST['autoassign']) && $_POST['autoassign'] == 'Y' ? 1 : 0;
    $_SESSION['cat_type'] = isset($_POST['type']) && $_POST['type'] == 'Y' ? 1 : 0;
	...
	$sql = "INSERT INTO `".hesk_dbEscape($hesk_settings['db_pfix'])."categories` (`name`,`cat_order`,`autoassign`,`type`) VALUES (
    '".hesk_dbEscape($catname)."',
    '".hesk_dbEscape($my_order)."',
    '".hesk_dbEscape($_SESSION['cat_autoassign'])."',
    '".hesk_dbEscape($_SESSION['cat_type'])."'
    )";
	$res = hesk_dbQuery($sql);
...
function rename_cat() {
    ...
    $_SERVER['PHP_SELF'] = 'manage_categories.php?catid='.intval($_POST['catid']);
...
function remove() {
	...
	$mycat = hesk_isNumber($_GET['catid'],$hesklang['no_cat_id']);
	if ($mycat == 1)
    {
...
function toggle_autoassign() {
	...
	$sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'categories` SET `autoassign`=\''.hesk_dbEscape($autoassign).'\' WHERE `id`='.hesk_dbEscape($catid).' LIMIT 1';
	$res = hesk_dbQuery($sql);
	if (hesk_dbAffectedRows() != 1)
    {
...
function toggle_type() {
	...
	/* Update auto-assign settings */
	$sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'categories` SET `type`=\''.hesk_dbEscape($type).'\' WHERE `id`='.hesk_dbEscape($catid).' LIMIT 1';
	$res = hesk_dbQuery($sql);
	if (hesk_dbAffectedRows() != 1)
    {
        hesk_process_messages($hesklang['int_error'].': '.$hesklang['cat_not_found'],'./manage_categories.php');
