...
$trackingID = hesk_cleanID( hesk_REQUEST('track') ) or print_form();
...
if (hesk_dbNumRows($res) != 1)
{
	...
	$sql = "SELECT * FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."tickets` WHERE `merged` LIKE '%#".hesk_dbEscape($trackingID)."#%' LIMIT 1";
	$res = hesk_dbQuery($sql);
...
if (isset($_GET['delete_post']) && $can_delete && hesk_token_check($_GET['token']))
{
    ...
    if ($n)
    {
		...
		$sql = "SELECT `attachments` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."replies` WHERE `id`=".hesk_dbEscape($n)." AND `replyto`=".hesk_dbEscape($ticket['id'])." LIMIT 1";
		$res = hesk_dbQuery($sql);
		...
		if ( strlen($attachments) )
		{
			...
			foreach ($att as $myatt)
			{
				...
				$sql = 'DELETE FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'attachments` WHERE `att_id`=\''.hesk_dbEscape($att_id).'\' LIMIT 1';
				hesk_dbQuery($sql);
			}
		}
        ...
        else
        {
			...
			if (hesk_dbNumRows($res))
			{
				...
				$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."tickets` SET `lastchange`=NOW(), `lastreplier`='".hesk_dbEscape($last_replier)."', `replierid`=".hesk_dbEscape($replier_id)." $status_sql WHERE `id`=".hesk_dbEscape($ticket['id'])." LIMIT 1";
				$res = hesk_dbQuery($sql);
			}
			else
			{
            	$status = $ticket['locked'] ? 3 : 0;
				$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."tickets` SET `lastchange`=NOW(), `lastreplier`='0', `status`='".hesk_dbEscape($status)."' WHERE `id`=".hesk_dbEscape($ticket['id'])." LIMIT 1";
				$res = hesk_dbQuery($sql);
			}
...
if (isset($_POST['notemsg']) && hesk_token_check($_POST['token']))
{
    ...
    if ($msg)
    {
        ...
        {
			$sql = "SELECT `email`, `notify_note` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."users` WHERE `id`=".hesk_dbEscape($ticket['owner'])." LIMIT 1";
			$res = hesk_dbQuery($sql);
...
if ( ($can_reply || $can_edit) && isset($_POST['h']) && isset($_POST['m']) && isset($_POST['s']) && hesk_token_check($_POST['token']))
{
	...
	$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."tickets` SET `time_worked`='" . hesk_dbEscape($time_worked) . "', `history`=CONCAT(`history`,'" . hesk_dbEscape($revision) . "') WHERE `trackid`='" . hesk_dbEscape($trackingID) . "' LIMIT 1";
	hesk_dbQuery($sql);
...
/* Get replies */
$reply = '';
$sql   = "SELECT * FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."replies` WHERE `replyto`='".hesk_dbEscape($ticket['id'])."' ORDER BY `id` ";
$sql  .= $hesk_settings['new_top'] ? 'DESC' : 'ASC';
$result  = hesk_dbQuery($sql);
