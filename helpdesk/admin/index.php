...
function do_login() {
    ...
    */
	...
	elseif ( isset($_POST['remember_user']) && $_POST['remember_user']=='JUSTUSER')
	{
		setcookie('hesk_username', "$user", strtotime('+1 year'));
		setcookie('hesk_p', '');
	}
...
function print_login() {
	...
	<tr>
        ...
        <tr>
				...
				{
					$savedUser = isset($_COOKIE['hesk_username']) ? hesk_htmlspecialchars($_COOKIE['hesk_username']) : '';
				}
				...
				if ($hesk_settings['list_users'])
				{
				    ...
				    $sql = 'SELECT * FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'users` ORDER BY `user` ASC';
