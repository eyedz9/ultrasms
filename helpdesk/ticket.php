    ...
    <!-- START TICKET HEAD -->
		...
		switch ($ticket['status'])
		{
			...
			default:
				...
				{
					echo ' [<a href="change_status.php?track='.$trackingID.$hesk_settings['e_query'].'&amp;s=1&amp;Refresh='.$random.'&amp;token='.hesk_token_echo(0).'">'.$hesklang['open_action'].'</a>]';
				}
		}
...
{
	$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."replies` SET `read` = '1' WHERE `id` IN (".hesk_dbEscape(implode(',', $unread_replies)).")";
	hesk_dbQuery($sql);
}
...
function hesk_printCustomerReplyForm()
{
