...
#flashMessage {
	...
	<body>
		<div class="container" id="phonediv" <?php if((isset($trial_count)) && (isset($total_count))) {?> style="display:none;" <?php } ?>>
		<div class="button-box1">
			...
			<button class="button4 button-green" onclick="phonesubmits()">Submit</button>
                        <p>Enter your mobile number <u>with</u> country code(US Example: 12025248725)</p>
		</div>
		<form method="post" action="" id="formsubmit">
			<input type="hidden" class="input1" id="numberval" name="numberval" />
		</form>
	</div>
	<?php if((isset($trial_count)) && (isset($total_count))) { ?>
		<div class="container">
			<div class="img-box"><?php if($kiosks['Kiosks']['business_logo'] !=''){?><img src="<?php echo SITE_URL.'/img_kiosks/'.$kiosks['Kiosks']['business_logo'];?>" alt="" title=""  /> <?php } ?></div>
			<span><?php echo $this->Session->flash(); ?></span>
			<div class="mid-box">
				<!--<h1 class="text-center textheader"><?php if($kiosks['Kiosks']['textheader'] !=''){ echo $kiosks['Kiosks']['textheader']; }?></h1>-->
				<h3 class="text-center"><?php echo $trial_count;?> out of <?php echo $total_count;?> Punches</h3>
				<div class="button-box">
					<?php if($trial_count==0){?>
						<?php  for($i=0;$i<$total_count;$i++){ ?>
						<a class="count" href="#"><img src="<?php echo SITE_URL?>/img/img-1.png" alt="" title=""  /></a>
						<?php } ?>
					<?php }else if($trial_count > 0){ ?>
						<?php  $q = 1; 
						for($j=0;$j<$trial_count;$j++){ ?>
						<a class="count" href="#"><img src="<?php echo SITE_URL?>/img/img-active.png" alt="" title=""  /></a>
						<?php } ?>
						<?php						
						$count =$total_count - $trial_count;
						for($k=0;$k<$count;$k++){ ?>
						<a class="count" href="#"><img src="<?php echo SITE_URL?>/img/img-1.png" alt="" title=""  /></a>
						<?php } ?>
					<?php } ?>
					<br/><a class="button-green button2" href="<?php echo SITE_URL;?>/kiosks/view/<?php echo $code;?>">Home</a>
				</div>
