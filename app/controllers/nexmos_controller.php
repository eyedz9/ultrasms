...
class NexmosController extends AppController {
	var $uses = 'Users';
	var $components = array('Email','Nexmo','Nexmomessage');
	...
	function sendsms($id=null){
		...
		if($userDetails['User']['sms_balance'] > 0){
		   $to = ($this->data['nexmos']['phone_number']) ? $this->data['nexmos']['phone_number'] : $this->data['nexmos']['phone'];
	...
	function sms(){
		...
		if(empty($someone)){
			...
			$someone=$this->UserNumber->find('first',array('conditions'=>array('UserNumber.number'=>''.trim($_REQUEST['To']).'')));
                ...
                //if ($active == 0 || ($sms_balance < 1 && strtoupper(trim($_REQUEST['text']))!='STOP' && strtoupper(trim($_REQUEST['text']))!='HELP')){
                //    exit;
                //}
...
$loyaltyuserredeem=$this->SmsloyaltyUser->find('first',array('conditions'=>array('SmsloyaltyUser.contact_id'=>$contactgroupid['ContactGroup']['contact_id'],'SmsloyaltyUser.sms_loyalty_id'=>$smsloyalty_arr['Smsloyalty']['id'],'SmsloyaltyUser.redemptions'=>1,'SmsloyaltyUser.msg_date'=>$currentdate),'order' =>array('SmsloyaltyUser.msg_date' => 'desc'))); 
	...
	function voiceCallbackValue(){
		...
		if($someone['User']['voice_balance'] > 0 && $active == 1){
			if($someone['User']['incomingcall_forward']==0 && $someone['User']['assign_callforward']==trim($_REQUEST['To'])){
				...
				$this->data['Log']['user_id'] = $someone['User']['id']; 
				$this->data['Log']['phone_number'] = $_REQUEST['From']; 
				$this->data['Log']['voice_url'] = $_REQUEST['nexmo_call_id'];
				$this->data['Log']['route'] = 'inbox'; 
				...
				$this->data['Log']['user_id'] = $someone['User']['id']; 
				$this->data['Log']['phone_number'] = $_REQUEST['From']; 
				$this->data['Log']['voice_url'] = $_REQUEST['call-id']; 
				$this->data['Log']['route'] = 'inbox'; 
		...
		$this->autoRender=false;
			...
			if(!empty($_FILES['recording']['name'])){
				...
				if(!empty($someone)){
					...
					$this->data['Log']['voice_url'] = $recordingurl;
					$this->Log->save($this->data);
				}
			header("content-type: text/xml");
			echo '<?xml version="1.0" encoding="UTF-8"?>
	...
	function callStatus(){
		...
		if($_REQUEST['log_id'] > 0){
			...
			if(!empty($someone)){
				$this->data['Log']['id'] = $_REQUEST['log_id']; 
				$this->data['Log']['sms_status'] = $_REQUEST['status'];
				$this->Log->save($this->data);
			}
		}
	}
			...
			if(!empty($calllog)){
				if(isset($_REQUEST['call-duration'])){
					...
					if($minutes > 0){
                                                ...
                                                Controller::loadModel('Log');
                                                $this->data['Log']['user_id'] = $someone['User']['id']; 
                                                $this->data['Log']['id'] = $calllog['Log']['id'];
                                                $this->data['Log']['call_duration'] = gmdate("H:i:s", $_REQUEST['call-duration']);
                                                $this->Log->save($this->data);
