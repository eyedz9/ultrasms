...
class InstantPaymentNotificationsController extends PaypalIpnAppController {
  ...
  function purchase_credit($package_id){
	  if($this->InstantPaymentNotification->isValid($_POST)){
      $notification = $this->InstantPaymentNotification->buildAssociationsFromIPN($_POST);
      $this->InstantPaymentNotification->saveAll($notification);
      $this->__processTransactionCredit($this->InstantPaymentNotification->id, $package_id);
	  }
	  $this->redirect('/');
  }

  private function __processTransactionCredit($txnId, $package_id){
    $this->log("Processing Trasaction: $txnId",'paypal');
    $this->log("package id: $package_id",'paypal');
    //Put the afterPaypalNotification($txnId) into your app_controller.php
    $this->afterPaypalNotificationCredit($txnId, $package_id);
  }

 function purchase_subscription($package_id){
	  if($this->InstantPaymentNotification->isValid($_POST)){
      $notification = $this->InstantPaymentNotification->buildAssociationsFromIPN($_POST);
      $this->InstantPaymentNotification->saveAll($notification);
      $this->__processTransactionSubscription($this->InstantPaymentNotification->id, $package_id);
	  }
	  $this->redirect('/');
          //$this->redirect(array('controller' =>'users', 'action'=>'profile'));
  }
	function purchase_subscription_numbers($package_id){
		if($this->InstantPaymentNotification->isValid($_POST)){
			$notification = $this->InstantPaymentNotification->buildAssociationsFromIPN($_POST);
			$this->InstantPaymentNotification->saveAll($notification);
			$this->__processTransactionSubscriptionnumbers($this->InstantPaymentNotification->id, $package_id);
		}
		$this->redirect('/');
		//$this->redirect(array('controller' =>'users', 'action'=>'profile'));
	}
	private function __processTransactionSubscriptionnumbers($txnId, $package_id){
		$this->log("Processing Trasaction: $txnId",'paypal');
		$this->log("package id: $package_id",'paypal');
		//Put the afterPaypalNotification($txnId) into your app_controller.php
		$this->afterPaypalNotificationSubscribenumbers($txnId, $package_id);
	}
  private function __processTransactionSubscription($txnId, $package_id){
    $this->log("Processing Trasaction: $txnId",'paypal');
    $this->log("package id: $package_id",'paypal');
    //Put the afterPaypalNotification($txnId) into your app_controller.php
    $this->afterPaypalNotificationSubscribe($txnId, $package_id);
  }
